...
class _CreatePromotionScreenState extends State<CreatePromotionScreen> {
  ...
  // Estado para as imagens, geolocalização e tipo de rolê
  ...
  Future<void> _handleGeocodingAndShowMap() async {
    FocusScope.of(context).unfocus();
    if (_addressController.text.isEmpty || _numberController.text.isEmpty || _postalCodeController.text.isEmpty) {
        ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Preencha o Logradouro, Número e CEP para buscar a localização.'))
        );
        return;
    }
    ...
    try {
        // CORREÇÃO: Enviando todos os campos que o backend espera para validação
        final addressData = {
        ...
        final initialCoords = await _apiService.getCoordinatesFromAddress(addressData);
        ...
        if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Erro ao buscar endereço: ${e.toString().replaceAll("Exception: ", "")}'))
            );
        }
    } finally {
  ...
  Future<void> _submitPromotion() async {
    ...
    if (_selectedImages.isEmpty) {
     ...
     if (_confirmedCoordinates == null) {
         ScaffoldMessenger.of(context).showSnackBar(
            const SnackBar(content: Text('Confirme a localização no mapa.'))
        );
        return;
    }
    ...
    try {
        // Etapa 1: Criar a promoção com todos os dados
        final promotionData = {
            "title": _titleController.text,
            "description": _descriptionController.text,
            "obs": _obsController.text,
            "promotionType": _selectedPromotionType?.name.toUpperCase(), // Envia o nome do enum em maiúsculo
            "active": true,
            "free": true,
            "address": {
                "address": _addressController.text,
                ...
                "postalCode": _postalCodeController.text,
                "obs": _addressObsController.text,
                // Adicione city e UF se os DTOs no backend forem atualizados
            },
            "latitude": _confirmedCoordinates!['latitude'],
            "longitude": _confirmedCoordinates!['longitude'],
        };
        
        final response = await _apiService.createPromotion(promotionData);
        final newPromotionId = response['user']['promotions'][0]['id'];
        
        // Etapa 2: Fazer o upload das imagens
        await _apiService.uploadPromotionImages(newPromotionId, _selectedImages);
        ...
        if (mounted) {
         ...
         if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
                SnackBar(content: Text('Erro ao cadastrar: ${e.toString().replaceAll("Exception: ", "")}'))
            );
        }
    } finally {
         if (mounted) setState(() => _isLoading = false);
            ...
            children: [
              ...
              if (_confirmedCoordinates != null)
                Padding(
                  ...
                  child: Row(
                    children: [
                      ...
                      TextButton(onPressed: _handleGeocodingAndShowMap, child: const Text('Ajustar'))
                    ],
                  ),
                )
              else 
                OutlinedButton.icon(
                  ...
                  label: _isLoading 
                    ...
                    : const Text('Buscar e confirmar no mapa'),
                  onPressed: _isLoading ? null : _handleGeocodingAndShowMap,
              ...
              ElevatedButton(
                ...
                child: _isLoading 
                  ...
                  : const Text('Salvar Rolê', style: TextStyle(color: Colors.white, fontSize: 16)),
              ),
            ],
          ),
        ),
  ...
  // ... (Os métodos _buildImageGrid, _buildAddImageButton e _buildTextField permanecem os mesmos)
  // Cole-os aqui a partir da resposta anterior.
    ...
    return Column(
      ...
      children: [
        ...
        TextFormField(
          ...
          validator: (value) {
            // Apenas valida se não for um campo de observação
            if (!label.toLowerCase().contains('observação') && !label.toLowerCase().contains('complemento') && (value == null || value.isEmpty)) {
